...
#endif
        if (!success) {
            TLog(@"Could not create database queue for path %@", aPath);
            FMDBRelease(self);
            return 0x00;
        }
...
#endif
        if (!success) {
            TLog(@"FMDatabaseQueue could not reopen database for path %@", _path);
            FMDBRelease(_db);
            _db  = 0x00;
            return 0x00;
        }
...
- (void)inDatabase:(void (^)(FMDatabase *db))block {
    ...
    dispatch_sync(_queue, ^() {
        ...
        if ([db hasOpenResultSets]) {
            TLog(@"Warning: there is at least one open result set around after performing [FMDatabaseQueue inDatabase:]");
